...
│                   ▼                                               │
│          ┌────────────────┐                                       │
│          │  fetch_daily.py │  신규 고시문 수집 + 중심지 매칭          │
│          └────────┬───────┘                                       │
│                   ▼                                               │
│          ┌────────────────┐                                       │
│          │  data/*.json    │  날짜별 JSON 저장                     │
│          └────────┬───────┘                                       │
│                   ▼                                               │
│  ┌─────────────────────────────────────────────────────────┐     │
│  │                  publish_daily.py                        │     │
│  │                                                          │     │
│  │  ┌──────────────────┐   ┌──────────────────┐            │     │
│  │  │ insight_generator │   │   thumbnail.py    │            │     │
│  │  │  (Claude Sonnet)  │   │  (PDF→PNG+오버레이) │            │     │
│  │  │                   │   │                   │            │     │
│  │  │ • 요약 2~3줄      │   │ • 등급별 컬러     │            │     │
│  │  │ • 영향 분석       │   │ • 지역명/제목     │            │     │
│  │  │ • 정책 연결       │   │ • PDF 첫페이지    │            │     │
...
│  │                    ▼                                     │     │
│  │           ┌──────────────────┐                           │     │
│  │           │  wp_publisher.py  │  WordPress REST API 발행  │     │
│  │           └──────────────────┘                           │     │
│  └─────────────────────────────────────────────────────────┘     │
│                   │                                               │
│                   ▼                                               │
│          ┌────────────────────┐                                   │
│          │ wp_published.json   │  발행 기록 (중복 방지)             │
│          └────────────────────┘                                   │
└─────────────────────────────────────────────────────────────────┘

         ┌──────────────┐              ┌──────────────────┐
         │  WordPress    │              │  Next.js 대시보드  │
         │  (2lee.kr)    │              │  (Vercel)         │
         │               │              │                   │
         │ • AI 해설 포함 │              │ • 중심지 지도     │
         │ • 카테고리 분류 │              │ • 필터/검색       │
